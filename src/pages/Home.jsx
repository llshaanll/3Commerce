  ...
  return (
    <div className="min-h-screen w-screen flex items-stretch">
      ...
      {!menuOpen && (
        <button 
          onClick={toggleMenu}
          className="fixed right-0 top-0 m-4 p-2 bg-white border rounded-md z-30"
        >
          ☰ Menu
        </button>
      )}
      ...
      <div className="border-1 flex-3 flex flex-wrap p-10 gap-5">
        {[...Array(10)].map((_, i) => (
          <ClothCard
            key={i}
            index={i}
  ...
  return (
    <div
      ...
      className={`md:h-[300px] md:w-[200px] flex flex-col relative
        group duration-300
        ${isHovered && hoveredIndex === index ? 'scale-110 z-20' : ''}
        ${isAnotherHovered ? 'pointer-events-none' : 'pointer-events-auto'}
      `}
    >
      <div
        className={`border-1 bg-white group-hover:shadow-xl/20 flex-4 group-hover:scale-105 duration-300 relative z-10`}
      >
        Canvas
      </div>
      <div
      ...
      >
        ...
        <div>
          <button onClick={()=> navigate('/cart')} className="border-1 w-full bg-white hover:bg-black hover:text-white group-hover:shadow-xl/30 duration-300">
            Add to cart
          </button>
        </div>
      </div>
